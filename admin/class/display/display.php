	...
	class display
	{
		...
		public function display_equipment()
		{
			...
			if($row > 0){
				...
				foreach ($fetch as $key => $value) {
					$str_status = ($value['item_status'] == 1) ? 'Company Pass' : 'Company Fail' ;
					$button = 	'<div class="btn-group">
				...
				$fetch = $sql->fetchAll();
					if($count > 0){
						...
						foreach ($fetch as $key => $value) {
							...
							$item_status = ($value['item_status'] == 1) ? 'Company Pass' : 'Company Fail';
		...
		public function display_equipmentinfo($id)
		{
			...
			if($count > 0){
				foreach ($fetch as $key => $value) {
					...
					$item_stat = ($value['item_status'] == 1 ) ? 'Company Pass' : 'Company Fail';
					$photo = ($value['i_photo'] == "") ? "../assets/noimagefound.jpg" : "../uploads/" . $value['i_photo'];
		...
		public function display_item_borrow()
		{
			...
			if($row > 0){
				foreach ($fetch as $key => $value) {
					$itemstatus = ($value['item_status'] == 1) ? 'Company Pass' : 'Company Fail';
		...
		public function table_dashboard()
		{
			...
			if($row > 0){
				foreach ($fetch as $key => $value) {
					$status = ($value['item_status'] == 1) ? 'Company Pass' : 'Company Fail' ;
